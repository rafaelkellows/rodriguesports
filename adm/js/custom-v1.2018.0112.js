	...
	var getColorpick = function(){
		$('.colorpick').ColorPicker({
			onSubmit: function(hsb, hex, rgb, el) {
				...
				$('.moreFields.colors ul li > input.color_label').show().focus().blur(function() {
				...
				$('select[name=labelColor] option:eq(0)').attr('selected','selected').closest('.jqTransformSelectWrapper').find('>div span').html( 'Selecione uma cor já cadastrada' );
				//$('select[name=labelColor]').prev('ul').find('li:first a').click();
			},
		...
		$('.moreFields.colors ul li > input.color_label').hide();
		$('.moreFields.colors ul li #labelColorContainer ul li').each(function(){
			$(this).append('<span style="background-color: #'+ $(this).find('a').attr('a-value') +' "></span>');
		})
	}
	getColorpick();
