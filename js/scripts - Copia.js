...
    });
	...
	function checkout(){
		...
		$('ul.size li, ul.color li').each(function(){
		...
		$('ul.color li').hover( 
			function(){
				...
				if( $(this).closest('ul').find('em font').length ){
					$(this).closest('ul').find('em font').remove();
				}
				$(this).closest('ul').find('em').append('<font>: '+$(this).attr('title')+'</font>');
			}, 
			function(){
				if($(this).closest('ul').find('li.active').length){
					$(this).closest('ul').find('em font').html(_lstColor.text());
					return;
				}
				///$(this).closest('ul').find('em font').html(_lstColor.text());
				$(this).closest('ul').find('em font').remove();
			...
			_lstColor = $(this).closest('ul').find('em font');
		});
		...
		$('.btn-buy.btn-color-E, .btn-default.btn-color-E').unbind('click').click(function(e){
			...
			if( !$('ul.size li').hasClass('active') ){
				...
				_tmlTitle = ( ( $('main.main_products > aside > div input[name=title]').length ) ? $('main.main_products > aside > div input[name=title]').val() : $(this).closest('dl').find('dt a').text() );
				_tmlTitle += ' - Tamanho: '+ $('ul.size li.active').text();
				//main.main_products section.product_features #owl-product_features dl dt a
				//_tmlTitle = $('main.main_products > aside > div input[name=title]').val() + ' - Tamanho: '+ $('ul.size li.active').text();
			}		
			if( !$('ul.color li').hasClass('active') ){
				...
				_tmlCor = ( $('ul.color li.active').attr('title') == '' ) ? $('ul.color li.active').text() : $('ul.color li.active').attr('title');
				_tmlTitle += ' - Cor: '+ _tmlCor;
			}		
			$('main.main_products > aside > div input[name=title]').val(_tmlTitle);
			...
			if( $(this).hasClass('btn-buy') ){
				...
				arrItens[arrItens.length-1].push( $(this).closest('dl').find('dt').attr('id'), $(this).closest('dl').find('dt a').text(), $(this).closest('dl').find('dd span.resume').text(), $(this).closest('dl').find('dd img').attr('src'), 1);
			}else{
			...
			if( data!=undefined ){
				...
				for(var x=0; x<data.length; x=x+1){
					...
					if(checkOutForm.length){
						var _tr = '<tr>';
							...
							_tr += '		<p>'+data[x][5]+'<br><span>Peso: '+data[x][3].substring(data[x][3].indexOf('=')+1)+' Kg</span><input id="itemDescription'+x+'" name="itemDescription'+x+'" type="hidden" value="'+data[x][5]+'"></p>';
							_tr += '	</td>';
							_tr += '	<td>';
				...
				setTimeout(function(){
				    ...
				    if(i_val>0){
						...
						$('form#checkout table tfoot tr.weight td > i').html(_t_weights.toFixed(3).replace('.',','));
					}
					//END Calculate
